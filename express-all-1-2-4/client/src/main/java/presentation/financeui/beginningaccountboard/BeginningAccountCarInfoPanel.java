...
public class BeginningAccountCarInfoPanel extends JPanel implements ActionListener{
	...
	static public JButton jbDriverInfo,jbCarInfo,jbAccountID,
	...
	public BeginningAccountCarInfoPanel(){
...
//				}
		...
		jlcurrentID = new JLabel("当前身份：高级财务人员 "+UserID.userid+" 期初建账:车辆");
		...
		for(int i = 0;i<carlist.size();i++){
			...
			row = new Vector<String>();
			row.add(0, vo.getCarNumber());
			row.add(1, vo.getPlateNumber());
			row.add(2, vo.getActiveTime()+"");
			model.addRow(row);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == jbstop){
			...
			boolean isNew = beginningAccountController.syncCar();
			if(isNew){
				model.setRowCount(0);
				ArrayList<CarInfoVO> carlist = beginningAccountController.findAlCarInfo();
				for(int i = 0;i<carlist.size();i++){
	...
	public static void setJbCarInfo(JButton jbCarInfo) {
		BeginningAccountCarInfoPanel.jbCarInfo = jbCarInfo;
	}
	...
	public static void setJbDriverInfo(JButton jbDriverInfo) {
		BeginningAccountCarInfoPanel.jbDriverInfo = jbDriverInfo;
	}
	...
	public static void setJbAccountID(JButton jbAccountID) {
		BeginningAccountCarInfoPanel.jbAccountID = jbAccountID;
	}
	...
	public static void setJbID(JButton jbID) {
		BeginningAccountCarInfoPanel.jbID = jbID;
	}
	...
	public static void setJbInstitutionsManage(JButton jbInstitutionsManage) {
		BeginningAccountCarInfoPanel.jbInstitutionsManage = jbInstitutionsManage;
	}
