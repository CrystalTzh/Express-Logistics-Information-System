...
import presentation.transitui.StoreArrivalBoard.StoreArrivalFrame;
import presentation.userui.modifypasswordui.ModifyPasswordBoard;

public class RelayArrivalFormNavigation extends JPanel implements ActionListener{
	...
	private ArrayList<StoreArrivalFormPO> allform;
	private DefaultTableModel model;
	private Vector<String> tableValues ;
	private Vector<String> row;
	
	public RelayArrivalFormNavigation(){
...
//				}
		...
		jlcurrentID = new JLabel("当前身份：中转中心业务员   "+UserID.userid+" 当前任务： 到达单");
...
//        table.setEnabled(false);
        ...
        allform = storeArrivalFormController.findAll();
        if(allform == null){
        	System.out.println("中转中心到达单信息为空！");
        }else {
        	for(int i = 0;i<allform.size();i++){
            	row = new Vector<String>();
            	
            	String NO = allform.get(i).getNO();
            	String date = allform.get(i).getArriveDate();
	...
	public static void setJbTransfer(JButton jbTransfer) {
		RelayArrivalFormNavigation.jbTransfer = jbTransfer;
	}
	...
	public static void setJbArrival(JButton jbArrival) {
		RelayArrivalFormNavigation.jbArrival = jbArrival;
	}
	...
	public static void setJbCarInput(JButton jbCarInput) {
		RelayArrivalFormNavigation.jbCarInput = jbCarInput;
	}
