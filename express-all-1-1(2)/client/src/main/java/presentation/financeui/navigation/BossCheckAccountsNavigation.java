...
import presentation.corporationui.bankaccountboard.BankAccountBoardInquire;
...
public class BossCheckAccountsNavigation extends JPanel implements ActionListener{
	...
	public BossCheckAccountsNavigation(JFrame frame){
		this.frame=frame;
		...
		jlcurrentID = new JLabel("当前身份：总经理      当前任务：查看账户");
		jlcurrentID.setFont(new Font("当前身份：总经理       当前任务：查看账户",Font.PLAIN,15));
...
//        sp.add(jpanel3);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == jbstart){
			BankAccountBoardInquire a =	new BankAccountBoardInquire(frame);
			a.setVisible(true);
		}
	}
