...
import presentation.transitui.TransferBoard.TransferFrame;
...
public class RelayFormNavigation extends JPanel implements ActionListener{
	...
	private ArrayList<TransferFormPO> allform;
...
//				}
		...
		jlcurrentID = new JLabel("当前身份：中转中心业务员   "+UserID.userid+" 当前任务：中转单");
...
//        table.setEnabled(false);
        ...
        allform = transferFormController.findAll();
        if(allform == null){
        	System.out.println("中转单信息为空！");
        }else {
        	for(int i = 0;i<allform.size();i++){
            	row = new Vector<String>();
            	
            	String NO = allform.get(i).getRelayFormNumber();
	...
	public static void setJbTransfer(JButton jbTransfer) {
		RelayFormNavigation.jbTransfer = jbTransfer;
	}
	...
	public static void setJbArrival(JButton jbArrival) {
		RelayFormNavigation.jbArrival = jbArrival;
	}
	...
	public static void setJbCarInput(JButton jbCarInput) {
		RelayFormNavigation.jbCarInput = jbCarInput;
	}
