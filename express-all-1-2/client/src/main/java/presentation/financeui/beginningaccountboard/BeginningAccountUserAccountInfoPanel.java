...
public class BeginningAccountUserAccountInfoPanel extends JPanel implements ActionListener{
	...
	public BeginningAccountUserAccountInfoPanel(){
...
//				}
		...
		jlcurrentID = new JLabel("当前身份：高级财务人员 "+UserID.userid+" 期初建账:用户");
        ...
        for(Iterator<UserRole> itr = userlist.keySet().iterator(); itr.hasNext();) {
			...
			for(int j = 0; j < list.size(); j++) {
				...
				row.add(1, vo.getUserName());
				row.add(2, vo.getUserRole().toString());
				model.addRow(row);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == jbstop){
			...
			boolean isNew = beginningAccountController.syncUserAccount();
			if(isNew){
				model.setRowCount(0);
				Hashtable<UserRole, ArrayList<UserAccountVO>> userlist = beginningAccountController.findAllUserAccountInfo();
				for(Iterator<UserRole> itr = userlist.keySet().iterator(); itr.hasNext();) {
	...
	public static void setJbCarInfo(JButton jbCarInfo) {
		BeginningAccountUserAccountInfoPanel.jbCarInfo = jbCarInfo;
	}
	...
	public static void setJbDriverInfo(JButton jbDriverInfo) {
		BeginningAccountUserAccountInfoPanel.jbDriverInfo = jbDriverInfo;
	}
	...
	public static void setJbAccountID(JButton jbAccountID) {
		BeginningAccountUserAccountInfoPanel.jbAccountID = jbAccountID;
	}
	...
	public static void setJbID(JButton jbID) {
		BeginningAccountUserAccountInfoPanel.jbID = jbID;
	}
	...
	public static void setJbInstitutionsManage(JButton jbInstitutionsManage) {
		BeginningAccountUserAccountInfoPanel.jbInstitutionsManage = jbInstitutionsManage;
	}
	
	public static void main(String[] args) {
		MainFrame main = new MainFrame();
		main.setContentPane(new BeginningAccountUserAccountInfoPanel());
