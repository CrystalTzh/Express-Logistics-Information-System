...
public class BeginningAccountDriverInfoPanel extends JPanel implements ActionListener {
	...
	public BeginningAccountDriverInfoPanel(){
...
//				}
		...
		jlcurrentID = new JLabel("当前身份：高级财务人员 "+UserID.userid+" 期初建账:司机");
		...
		for(int i = 0;i<driverlist.size();i++){
			...
			row = new Vector<String>();
			row.add(0, vo.getName());
			row.add(1, vo.getDriverNumber());
			row.add(2, vo.getSex().toString());
			row.add(3, vo.getBirthday());
			row.add(4, vo.getCellphone());
			row.add(5, vo.getLicensedate());
			model.addRow(row);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == jbstop){
			...
			boolean isNew = beginningAccountController.syncDriver();
			if(isNew){
				model.setRowCount(0);
				 ArrayList<DriverInfoVO> driverlist = beginningAccountController.findAllDriverInfo();
					for(int i = 0;i<driverlist.size();i++){
	...
	public static void setJbCarInfo(JButton jbCarInfo) {
		BeginningAccountDriverInfoPanel.jbCarInfo = jbCarInfo;
	}
	...
	public static void setJbDriverInfo(JButton jbDriverInfo) {
		BeginningAccountDriverInfoPanel.jbDriverInfo = jbDriverInfo;
	}
	...
	public static void setJbAccountID(JButton jbAccountID) {
		BeginningAccountDriverInfoPanel.jbAccountID = jbAccountID;
	}
	...
	public static void setJbID(JButton jbID) {
		BeginningAccountDriverInfoPanel.jbID = jbID;
	}
	...
	public static void setJbInstitutionsManage(JButton jbInstitutionsManage) {
		BeginningAccountDriverInfoPanel.jbInstitutionsManage = jbInstitutionsManage;
	}
	
	public static void main(String[] args) {
		MainFrame main = new MainFrame();
		main.setContentPane(new BeginningAccountDriverInfoPanel());
